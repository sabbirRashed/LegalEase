"use client"
import { motion } from "framer-motion";
import Image from "next/image";
import Link from "next/link";
import { FiArrowRight } from "react-icons/fi";

const TopLawyerCard = ({ lawyer }) => {

    const { _id, name, imageUrl, specialization, hourlyRate, status } = lawyer;

    return (
        <motion.div
            initial={{ opacity: 0, y: 60 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.2 }}
            whileHover={{
                scale: 1.03,
                y: -4,
            }}
            transition={{
                duration: 0.5,
                ease: "easeOut",
            }}
            className="relative flex flex-col flex-1 items-center rounded-2xl border border-slate-200 bg-white p-3 sm:p-6 text-center shadow-sm transition-shadow hover:shadow-lg hover:shadow-slate-200/60"
        >

            {/* Avatar */}
            <div className="relative">
                <div className="h-18 sm:h-24 w-18 sm:w-24 overflow-hidden rounded-full border-4 border-slate-50 ring-1 ring-slate-200">

                    <Image
                        src={imageUrl}
                        alt={name}
                        width={96}
                        height={96}
                        className="h-full w-full object-cover"
                    >
                    </Image>
                </div>
            </div>

            {/* Name */}
            <h3 className="mt-3 sm:mt-4 text-md sm:text-lg font-bold text-slate-900">{name}</h3>

            {/* Specialization */}
            <span className="mt-1.5 rounded-full bg-blue-50 px-3 py-1 text-[10px] sm:text-xs font-semibold text-blue-600">
                {specialization}
            </span>

            {/* Hourly rate + CTA */}
            <div className="mt-3 sm:mt-5 w-full items-center justify-between border-t border-slate-100 pt-2 sm:pt-4">

                <Link
                    href={`/lawyers/lawyerDetails/${_id}`}
                    className="w-full  flex justify-center items-center gap-1.5 rounded-lg bg-blue-600 px-3.5 py-3 text-xs font-semibold text-white transition-colors hover:bg-blue-700 "
                >
                    View Profile
                    <FiArrowRight className="h-3.5 w-3.5" />
                </Link>
            </div>
        </motion.div>
    );
};

export default TopLawyerCard;